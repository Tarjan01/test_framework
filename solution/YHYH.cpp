...
/**************************************************************
 * YHYH structure implementation
 **************************************************************/

int YHYH::nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no + 1] -
            cum_nneighbor_per_level[layer_no];
}

void YHYH::set_nb_neighbors(int level_no, int n) {
...
int YHYH::cum_nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no];
}

void YHYH::neighbor_range(idx_t no, int layer_no, size_t* begin, size_t* end)
        const {
    size_t o = offsets[no];
...
YHYH::YHYH(int M) : rng(12345) {
    set_default_probas(M, 1.0 / log(M));
    offsets.push_back(0);
}

int YHYH::random_level() {
...
void YHYH::set_default_probas(int M, float levelMult) {
    int nn = 0;
...
void YHYH::clear_neighbor_tables(int level) {
...
void YHYH::print_neighbor_stats(int level) const {
...
void YHYH::fill_with_random_links(size_t n) {
...
int YHYH::prepare_level_tab(size_t n, bool preset_levels) {
 ...
 */
void YHYH::shrink_neighbor_list(
...
using NodeDistCloser = YHYH::NodeDistCloser;
...
void shrink_neighbor_list(
    ...
    YHYH::shrink_neighbor_list(qdis, resultSet, returnlist, max_size);
...
void add_link(
        ...
        int level) {
    size_t begin, end;
    yhyh.neighbor_range(src, level, &begin, &end);
    if (yhyh.neighbors[end - 1] == -1) {
        // there is enough room, find a slot to add it
        size_t i = end;
        while (i > begin) {
            if (yhyh.neighbors[i - 1] != -1)
                break;
            i--;
        }
        yhyh.neighbors[i] = dest;
        return;
    }
    ...
    for (size_t i = begin; i < end; i++) { // HERE WAS THE BUG
        storage_idx_t neigh = yhyh.neighbors[i];
    ...
    while (resultSet.size()) {
        yhyh.neighbors[i++] = resultSet.top().id;
        resultSet.pop();
    }
...
void search_neighbors_to_add(
    ...
    while (!candidates.empty()) {
        ...
        // loop over neighbors
        size_t begin, end;
        yhyh.neighbor_range(currNode, level, &begin, &end);
        for (size_t i = begin; i < end; i++) {
            storage_idx_t nodeId = yhyh.neighbors[i];
            if (nodeId < 0)
                break;
            if (vt.get(nodeId))
                continue;
            ...
            if (results.size() < yhyh.efConstruction || results.top().d > dis) {
                results.emplace(dis, nodeId);
                candidates.emplace(dis, nodeId);
                if (results.size() > yhyh.efConstruction) {
                    results.pop();
                }
            }
        }
...
void greedy_update_nearest(
    ...
    for (;;) {
        ...
        size_t begin, end;
        yhyh.neighbor_range(nearest, level, &begin, &end);
        for (size_t i = begin; i < end; i++) {
            storage_idx_t v = yhyh.neighbors[i];
            if (v < 0)
                break;
            float dis = qdis(v);
            if (dis < d_nearest) {
...
void YHYH::add_links_starting_from(
...
int search_from_candidates(
    ...
    bool do_dis_check = params ? params->check_relative_distance
                               : yhyh.check_relative_distance;
    int efSearch = params ? params->efSearch : yhyh.efSearch;
...
std::priority_queue<YHYH::Node> search_from_candidate_unbounded(
        const YHYH& yhyh,
        const Node& node,
        DistanceComputer& qdis,
        int ef,
    ...
    while (!candidates.empty()) {
        ...
        size_t begin, end;
        yhyh.neighbor_range(v0, 0, &begin, &end);
...
YHYHStats YHYH::search(
        DistanceComputer& qdis,
        ResultHandler<C>& res,
        VisitedTable& vt,
        const SearchParametersYHYH* params) const {
    YHYHStats stats;
    if (entry_point == -1) {
        return stats;
    }
...
void YHYH::search_level_0(
    ...
    if (search_type == 1) {
        ...
        for (int j = 0; j < nprobe; j++) {
            ...
            int candidates_size = std::max(yhyh.efSearch, k);
            MinimaxHeap candidates(candidates_size);

            candidates.push(cj, nearest_d[j]);

            nres = search_from_candidates(
                    yhyh, qdis, res, candidates, vt, search_stats, 0, nres);
        }
    } else if (search_type == 2) {
        int candidates_size = std::max(yhyh.efSearch, int(k));
        ...
        search_from_candidates(
                yhyh, qdis, res, candidates, vt, search_stats, 0);
    }
}

void YHYH::permute_entries(const idx_t* map) {
    // remap levels
...
void YHYH::MinimaxHeap::push(storage_idx_t i, float v) {
    if (k == n) {
        if (v >= dis[0])
            return;
        if (ids[0] != -1) {
...
float YHYH::MinimaxHeap::max() const {
    return dis[0];
}

int YHYH::MinimaxHeap::size() const {
    return nvalid;
}

void YHYH::MinimaxHeap::clear() {
    nvalid = k = 0;
}

#ifdef __AVX2__
int YHYH::MinimaxHeap::pop_min(float* vmin_out) {
    assert(k > 0);
    static_assert(
...
int YHYH::MinimaxHeap::count_below(float thresh) {
    int n_below = 0;
    for (int i = 0; i < k; i++) {
        if (dis[i] < thresh) {
            n_below++;
